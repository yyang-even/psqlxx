...
void Command::Help() const {
    // std::cout << names << '\t' << arguments << '\t' << description << std::endl;
}
...
CommandResult Command::operator()(const char **words,
    ...
    if (number_arguments > arguments.size()) {
        std::cerr << "Command (" << words[0] << ") failed: Too many arguments. Expected " <<
                  arguments.size() << ", but " << number_arguments << " were given." << std::endl;
        return CommandResult::failure;
    }

    return action(words, word_count);
}
